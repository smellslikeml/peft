...
"""Fused DoRA *compose* (forward) kernel — Stage B Triton fast-path.

Ports the compose kernel **verbatim** from the paper's reference implementation at
``sockeye44/dorafactors/code/kernelagent_sols/optimize_dora_compose/beam_w0_r2_kernel_round_1.py``
(equation: the fused magnitude-rescale + weight-compose of DoRA's effective weight,
arXiv:2603.22276 §3.2). The ``@triton.jit`` body, its ``@triton.autotune`` configs, and its
``@triton.heuristics`` predicates are kept byte-for-byte identical to that source — the autotune
configs are load-bearing for the paper's reported performance and re-tuning them is explicitly out
of scope for Stage B. Only the Python launch wrapper is renamed (``kernel_function`` →
:func:`dora_compose`) to fit the package layout; its launch logic is otherwise unchanged.

What the kernel fuses
---------------------

Given the column-wise DoRA magnitude scale ``mag`` (i.e. ``dora_scale / ||W + s·BA||``, one value per
output feature) and the dense LoRA delta ``BA``, the kernel computes the per-element *compose delta*

    compose_delta = (mag - 1) ⊙ base + (mag · COEFF) ⊙ lora

with ``COEFF = 0.7`` baked into the kernel body, so that the caller recovers the full DoRA effective
weight as ``W_eff = base + compose_delta``. Expanding::

    base + compose_delta = mag ⊙ base + (mag · COEFF) ⊙ lora = mag ⊙ (base + COEFF · lora)

i.e. the standard DoRA rescale ``mag ⊙ (W + s·BA)`` once the caller maps ``COEFF · lora`` onto
``s · BA``. That mapping is the wrapper's responsibility (see :mod:`autograd`): the caller folds
``scaling / 0.7`` into the dense ``BA`` it hands the kernel, so the kernel's literal ``0.7`` cancels
and the result is correct for *any* LoRA ``scaling`` — without touching the byte-for-byte kernel body.

Orientation
-----------

``mag`` is applied per **column** (``num_cols``); in PEFT the magnitude is per output feature
(``d_out``), so the caller feeds the transposed weight ``[d_in, d_out]`` so that ``num_cols == d_out``
lines up with ``mag``. The launch wrapper asserts ``mag.numel() == num_cols``.
"""
...
def dora_compose(
    ...
    """Launch :func:`_fused_dora_compose_kernel` over a 2D weight tile.

    Renamed wrapper (upstream: ``kernel_function``); launch logic unchanged. Reshapes the inputs to
    2D ``[num_rows, num_cols]`` (the magnitude scale indexes ``num_cols``), allocates the output, and
    launches the autotuned 1-D grid. Returns the *compose delta* — the caller adds ``base`` to
    recover the DoRA effective weight (see :mod:`autograd`).

    Args:
        lora: Dense LoRA delta ``[num_rows, num_cols]`` (caller pre-folds ``scaling / 0.7``).
        base: Base weight ``[num_rows, num_cols]``.
